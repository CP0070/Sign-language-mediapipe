...
def update_gestures(accumulated_gestures, last_gesture_time, gesture, gesture_cooldown):
    ...
    gesture_display_time = 7  # Gestures older than 5 seconds will be cleared from the screen
...
def main():
    ...
    with mp_hands.Hands(
        ...
        while cap.isOpened():
            ...
            key = cv2.waitKey(1)
            if key == 32:  # Space key
                accumulated_gestures.append('')
            elif key == ord('d'):  # 'd' key
                accumulated_gestures.pop()
            elif key == ord('q'):
                break
...
# Function for handling gesture accumulation, space insertion, and deletion
